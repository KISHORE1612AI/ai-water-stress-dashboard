    ...
    with col2:
        with card("<span class='sensor-title'> Prediction Results</span>"):
            ...
            if predict:
                ...
                st.plotly_chart(class_probability_bar(le.classes_, proba), width='stretch')
...
with tab2:
    ...
    with col1:
        st.plotly_chart(radar_metrics(data), width='stretch')

    with col2:
        cm = np.array(data["confusion_matrix"])
        st.plotly_chart(confusion_matrix_animated(cm, list(le.classes_)), width='stretch')
...
with tab3:
    ...
    st.plotly_chart(class_distribution_donut(df["Plant_Health_Status"]), width='stretch')

    feat = st.selectbox("Feature", feature_names)

    colA, colB = st.columns(2)
    colA.plotly_chart(hist_with_box(df, feat, "Plant_Health_Status"), width='stretch')
    colB.plotly_chart(box_by_class(df, feat, "Plant_Health_Status"), width='stretch')

    st.markdown("#### Correlation Heatmap")
    num = df[feature_names]
    st.plotly_chart(correlation_heatmap(num), width='stretch')
